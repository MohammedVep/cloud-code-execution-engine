...
| --- | --- | --- | --- |
| New language runtime | `packages/common/src/index.ts`, `services/runner/src/language-profiles.ts`, `services/runner/Dockerfile`, `services/api/public/index.html` | Add language schema, runtime profile, packages, and editor template | Submit/poll one successful job and one failing job |
| New admin dashboard metric | `services/api/src/index.ts`, `services/api/public/index.html` | Add metric to `/v1/admin/metrics`, render card/timeline row | Load `/admin/observability`, verify metric updates |
...
4. Add an editor template and Monaco language mapping in `services/api/public/index.html`.
...
3. Add one visual element in `services/api/public/index.html`.
